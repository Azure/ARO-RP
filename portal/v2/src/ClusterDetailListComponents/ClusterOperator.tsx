...
const renderClusterOperators = (operator: IClusterOperator) => {
    return <PivotItem key={operator.name} headerText={operator.name}>
                <Stack>
                ...
                <Stack wrap styles={stackItemStyles}horizontal grow>
                    <MultiInfoList headers={ConditionDetails} items={operator.conditions} title="Conditions" subProp="type" titleSize="medium"/>
                </Stack>
            </PivotItem>;
};
...
function Operators(props: {
    ...
    for(const operator of props.clusterOperators) {
        if (operator.name === props.clusterOperatorName) {
            return (
                <>
                        {renderClusterOperators(operator)}
                </>
            );
        }
    }
...
export class ClusterOperatorsComponent extends Component<ClusterOperatorComponentProps, IClusterOperatorsState> {
    ...
    public render() {        
        ...
        <Stack styles={contentStackStylesNormal}>
            <Stack>
                <Operators clusterOperators={this.state.clusterOperators} clusterOperatorName={this.state.clusterOperatorName}/>
            </Stack>
        </Stack>
        )
    }
