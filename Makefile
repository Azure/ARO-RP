...
unit-test-go:
	go run gotest.tools/gotestsum@v1.11.0 --format pkgname --junitfile report.xml -- -coverprofile=cover.out ./...

unit-test-go-coverpkg:
	go run gotest.tools/gotestsum@v1.11.0 --format pkgname --junitfile report.xml -- -coverpkg=./... -coverprofile=cover_coverpkg.out ./...
