...
**Development mode** (`RP_MODE=development`): The API base URL and UAMI client ID are read from environment variables (`HOLMES_AZURE_API_BASE`, `HOLMES_UAMI_CLIENT_ID`). This uses `NewHolmesConfigFromEnv(acrDomain)`.
