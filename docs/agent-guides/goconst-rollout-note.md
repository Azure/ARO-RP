...
PR1 already switches the workflow to source the `golangci-lint` version from bingo, which keeps the version pinned to the repo-managed tool definition. PR2 does not replace `golangci-lint-action` because the action still provides the CI wrapper that installs and runs the pinned version cleanly in GitHub Actions, along with the existing caching and reporting behavior. This rollout only changes lint configuration; it does not require a custom plugin or custom workflow execution path.
...
The thresholds are chosen empirically from a worktree run, generated and generated-like paths stay excluded, and the workflow continues to use golangci-lint-action with the version now sourced from bingo via PR1.
```
