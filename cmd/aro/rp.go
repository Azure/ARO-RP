...
func rp(ctx context.Context, log *logrus.Entry) error {
	...
	bundle, err = kv.GetSecret(ctx, env.FrontendEncryptionSecretName, "")
	if err != nil {
		return err
	}

	feKey, err := base64.StdEncoding.DecodeString(*bundle.Value)
	if err != nil {
		return err
	}
	...
	b, err := backend.NewBackend(ctx, log.WithField("component", "backend"), _env, fp, gl, dialer, fakearm, versions, zones, dbasyncoperations, dbbilling, dbopenshiftclusters, dbsubscriptions, cipher, m, clustersKeyvaultURI)
	if err != nil {
		return err
	}
