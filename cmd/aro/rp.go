...
func rp(ctx context.Context, log, audit *logrus.Entry) error {
	...
	dbBackend, err := database.NewBackends(ctx, _env.IsLocalDevelopmentMode(), dbc)
	if err != nil {
		return err
	}
	...
	b, err := backend.NewBackend(ctx, log.WithField("component", "backend"), _env, dbAsyncOperations, dbBilling, dbGateway, dbOpenShiftClusters, dbSubscriptions, dbOpenShiftVersions, dbBackend, dbClusterManagerConfiguration, aead, m)
	if err != nil {
		return err
	}
