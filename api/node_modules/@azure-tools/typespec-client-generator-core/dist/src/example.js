import { NoTarget, createDiagnosticCollector, getAnyExtensionFromPath, getRelativePathFromDirectory, joinPaths, normalizePath, resolvePath, } from "@typespec/compiler";
 ...
 */
async function loadExamples(context) {
    const diagnostics = createDiagnosticCollector();
    const apiVersions = context.getPackageVersions();
    const exampleDirs = [];
    if (apiVersions.size <= 1) {
        // single service case
        const apiVersion = apiVersions.size === 1 ? apiVersions.values().next().value?.at(-1) : undefined;
        const examplesBaseDir = resolvePath(context.program.projectRoot, context.examplesDir ?? "./examples");
        ...
        for (const [service, versions] of apiVersions) {
            ...
            const examplesBaseDir = resolvePath(context.program.projectRoot, service.name, context.examplesDir ?? "./examples");
            const exampleDir = apiVersion
                ? resolvePath(examplesBaseDir, apiVersion)
                : resolvePath(examplesBaseDir);
            if (await checkExamplesDirExists(context.program.host, exampleDir)) {
        ...
        for (const fileName of exampleFiles) {
            try {
                ...
                examples[example.title] = {
                    relativePath: getRelativePathFromDirectory(examplesBaseDir, resolvePath(exampleDir, fileName), false),
                    data: example,
                };
            }
            catch (err) {
