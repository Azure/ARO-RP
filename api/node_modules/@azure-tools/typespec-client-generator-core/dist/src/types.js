...
import { createDiagnosticCollector, getDiscriminator, getEncode, getLifecycleVisibilityEnum, getSummary, getVisibilityForClass, ignoreDiagnostics, isArrayModelType, isErrorModel, isNeverType, isTemplateDeclaration, resolveEncodedName, } from "@typespec/compiler";
...
import { getAccess, getAccessOverride, getAlternateType, getClientDefaultValue, getClientNamespace, getExplicitClientApiVersions, getLegacyHierarchyBuilding, getOverriddenClientMethod, getUsageOverride, isInScope, listClients, listOperationGroups, listOperationsInOperationGroup, shouldFlattenProperty, shouldGenerateConvenient, } from "./decorators.js";
import { UsageFlags, isSdkIntKind, } from "./interfaces.js";
import { createGeneratedName, filterPreviewVersion, getAvailableApiVersions, getClientDoc, getHttpBodyType, getHttpOperationResponseHeaders, getNonNullOptions, getNullOption, getSdkTypeBaseHelper, getTypeDecorators, hasNoneVisibility, intOrFloat, isHttpBodySpread, isNeverOrVoidType, isOnClient, listAllUserDefinedNamespaces, resolveConflictGeneratedName, updateWithApiVersionInformation, } from "./internal-utils.js";
...
export function addEncodeInfo(context, type, propertyType, defaultContentType) {
    ...
    if (innerType.kind === "bytes") {
        ...
        else if (!isMediaTypeJson(defaultContentType) &&
            ...
            if (retval === undefined) {
                ...
                if (discriminatedOptions) {
                    ...
                    retval.discriminatedOptions = {
                        ...
                        envelopePropertyName: envelope === "none"
                            ? undefined
    ...
    if (!sdkType) {
        ...
        if (operation) {
            ...
            if (requestBody &&
                ...
                multipartResponseBodies.map((body) => getHttpBodyType(body) === type
                    ? diagnostics.pipe(addMultipartPropertiesToModelType(context, sdkType, body, operation))
                    : undefined);
            }
            else {
...
export function getClientTypeWithDiagnostics(context, type, operation) {
    ...
    switch (type.kind) {
        ...
        case "Scalar":
            ...
            retval = diagnostics.pipe(getSdkDateTimeOrDurationOrBuiltInType(context, scalarAlternateType && scalarAlternateType.kind === "Scalar" ? scalarAlternateType : type));
            break;
        case "Enum":
...
export function getSdkModelPropertyTypeBase(context, type, operation) {
    ...
    const propertyType = getSdkTypeFromAlternateType(context, type, operation) ||
        diagnostics.pipe(getClientTypeWithDiagnostics(context, type.type, operation));
    ...
    return diagnostics.wrap({
        ...
        ...updateWithApiVersionInformation(context, type, operation ? context.getClientForOperation(operation) : undefined, operation),
        onClient,
        crossLanguageDefinitionId: getCrossLanguageDefinitionId(context, type, operation),
    ...
    for (const part of body.parts) {
        ...
        clientProperty.serializationOptions.multipart = {
            ...
            contentType: part.body.contentTypeProperty
                ? diagnostics.pipe(getSdkModelPropertyType(context, part.body.contentTypeProperty, operation))
                : undefined,
...
export function updateUsageOrAccess(context, value, type, options) {
    const diagnostics = createDiagnosticCollector();
    options = options ?? {};
    options.seenTypes = options.seenTypes ?? new Set();
...
function updateTypesFromOperation(context, operation) {
    ...
    if (httpBody && !isNeverOrVoidType(httpBody.type)) {
        ...
        const sdkType = diagnostics.pipe(getClientTypeWithDiagnostics(context, bodyTypeOrProperty, operation));
        ...
        for (const innerResponse of response.responses) {
            if (innerResponse.body?.type && !isNeverOrVoidType(innerResponse.body.type)) {
                ...
                if (generateConvenient) {
                    ...
                    // add serialization options to model type
                    updateSerializationOptions(context, sdkType, innerResponse.body.contentTypes, undefined, innerResponse.body);
                }
                const access = getAccessOverride(context, operation) ?? "public";
                diagnostics.pipe(updateUsageOrAccess(context, access, sdkType));
            }
...
function updateExternalUsage(context) {
 ...
 */
function hasInputOrOutputUsage(usage) {
 ...
 */
function cleanupDiscriminatorForUnusedBase(context) {
    for (const sdkType of context.__referencedTypeCache.values()) {
        if (sdkType.kind !== "model" || !sdkType.discriminatedSubtypes)
            continue;
...
function updateSerializationOptions(context, type, contentTypes, options, httpBody) {
    options = options ?? {};
    options.seenTypes = options.seenTypes ?? new Set();
    options.propagation = options?.propagation ?? true;
    options.ignoreSubTypeStack = options.ignoreSubTypeStack ?? [];
...
function setSerializationOptions(context, type, contentTypes) {
    ...
    if (!type.serializationOptions.json &&
        type.__raw &&
...
function updateJsonSerializationOptions(context, type) {
    type.serializationOptions.json = {
        name: type.__raw?.kind === "Model" || type.__raw?.kind === "ModelProperty"
...
function updateXmlSerializationOptions(context, type) {
    ...
    if (type.__raw?.kind === "ModelProperty" &&
        ...
        if (!type.serializationOptions.xml.unwrapped) {
            ...
            if ("name" in itemType) {
                ...
                type.serializationOptions.xml.itemsName = resolveEncodedName(context.program, itemType, "application/xml");
...
function getSdkTypeFromAlternateType(context, type, operation) {
    const alternateType = getAlternateType(context, type);
