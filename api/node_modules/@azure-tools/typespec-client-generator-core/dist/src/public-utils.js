...
import { getHttpBodyType, getHttpOperationResponseHeaders, hasExplicitClientOrOperationGroup, hasNoneVisibility, isAzureCoreTspModel, listAllUserDefinedNamespaces, removeVersionsLargerThanExplicitlySpecified, resolveDuplicateGenearatedName, } from "./internal-utils.js";
/**
...
export function isApiVersion(context, type) {
    ...
    return (versionEnumSets.some((versionEnum) => type.type === versionEnum) ||
...
export function getCrossLanguageDefinitionId(context, type, operation, appendNamespace = true) {
    let retval = typeof type.name === "symbol" ? "anonymous" : type.name || "anonymous";
    let namespace = type.kind === "ModelProperty"
        ...
        if (currentType === expectedType) {
            ...
            if (baseModel) {
                if (baseModel.name === "Record") {
                    const result = dfsModelProperties(expectedType, baseModel.indexer.value, "AdditionalProperty");
                    if (result)
                        return true;
                }
            }
 ...
 * @returns
 */
export function getHttpOperationParametersForClientParameter(client, param) {
    const result = [];
 ...
 * @returns
 */
export function isHttpMetadata(context, property) {
    return property.__raw !== undefined && isMetadata(context.program, property.__raw);
}
export function getNamespaceFromType(type) {
    if (type === undefined) {
        return undefined;
    }
 ...
 * ```
 */
export function getClientOptions(type, key) {
