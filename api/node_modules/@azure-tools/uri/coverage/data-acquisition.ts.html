...
<head>
    <title>Code coverage report for data-acquisition.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> data-acquisition.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">2x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import * as getUri from "get-uri";
import { toRawDataUrl } from "./uri-manipulation";
import { Readable } from "stream";
&nbsp;
function stripBom(text: string): string {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (text.charCodeAt(0) === 0xfeff) {
<span class="cstat-no" title="statement not covered" >    return text.slice(1);</span>
  }
  return text;
}
&nbsp;
...
&nbsp;
  const readAll = new Promise&lt;Buffer&gt;(function (resolve, reject) {
    ...
    readable.on("error", <span class="fstat-no" title="function not covered" >(e</span>rr) =&gt; <span class="cstat-no" title="statement not covered" >reject(err))</span>;
  });
&nbsp;
  const result = await readAll;
&nbsp;
  // make sure we can read 4 bytes into the file before trying to fix it!
  <span class="missing-if-branch" title="else path not taken" >E</span>if (result.length &gt; 3) {
    // fix up UTF16le files
    <span class="missing-if-branch" title="if path not taken" >I</span>if (result.readUInt16LE(0) === 65533 &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >result.readUInt16LE(1) === 65533)</span> {
<span class="cstat-no" title="statement not covered" >      return stripBom(result.slice(2).toString("utf16le"));</span>
    }
  }
