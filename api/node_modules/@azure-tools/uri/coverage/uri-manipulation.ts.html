...
<head>
    <title>Code coverage report for uri-manipulation.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> uri-manipulation.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">3x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">/* eslint-disable no-useless-escape */
...
&nbsp;
export function <span class="fstat-no" title="function not covered" >isUri(</span>uri: string): boolean {
<span class="cstat-no" title="statement not covered" >  return /^([a-z0-9+.-]+):(?:\/\/(?:((?:[a-z0-9-._~!$&amp;'()*+,;=:]|%[0-9A-F]{2})*)@)?((?:[a-z0-9-._~!$&amp;'()*+,;=]|%[0-9A-F]{2})*)(?::(\d*))?(\/(?:[a-z0-9-._~!$&amp;'()*+,;=:@/]|%[0-9A-F]{2})*)?|(\/?(?:[a-z0-9-._~!$&amp;'()*+,;=:@]|%[0-9A-F]{2})+(?:[a-z0-9-._~!$&amp;'()*+,;=:@/]|%[0-9A-F]{2})*)?)(?:\?((?:[a-z0-9-._~!$&amp;'()*+,;=:/?@]|%[0-9A-F]{2})*))?(?:#((?:[a-z0-9-._~!$&amp;'()*+,;=:/?@]|%[0-9A-F]{2})*))?$/i.test(</span>
    uri,
  );
}
&nbsp;
...
export function createFileOrFolderUri(absolutePath: string): string {
  ...
  // handle UNCs
  <span class="missing-if-branch" title="if path not taken" >I</span>if (absolutePath.startsWith("//") || absolutePath.startsWith("\\\\")) {
<span class="cstat-no" title="statement not covered" >    result = result.replace(/^file:\/\/\/\//, "file://");</span>
  }
  return result;
}
&nbsp;
...
&nbsp;
export function <span class="fstat-no" title="function not covered" >getFilename(</span>uri: string): string {
<span class="cstat-no" title="statement not covered" >  return uri.split("/").reverse()[0].split("\\").reverse()[0];</span>
}
&nbsp;
export function <span class="fstat-no" title="function not covered" >getFilenameWithoutExtension(</span>uri: string): string {
  const lastPart = <span class="cstat-no" title="statement not covered" >getFilename(uri);</span>
  const ext = <span class="cstat-no" title="statement not covered" >lastPart.indexOf(".") === -1 ? "" : lastPart.split(".").reverse()[0];</span>
<span class="cstat-no" title="statement not covered" >  return lastPart.substr(0, lastPart.length - ext.length - 1);</span>
}
&nbsp;
...
&nbsp;
  ...
  // - GitHub gist
  <span class="missing-if-branch" title="if path not taken" >I</span>if (uri.startsWith("gist://")) {
<span class="cstat-no" title="statement not covered" >    uri = uri.replace(/^gist:\/\/([^\/]+\/[^\/]+)$/gi, "https://gist.githubusercontent.com/$1/raw/");</span>
  }
  <span class="missing-if-branch" title="if path not taken" >I</span>if (uri.startsWith("https://gist.github.com")) {
<span class="cstat-no" title="statement not covered" >    uri = uri.replace(/^https?:\/\/gist.github.com\/([^\/]+\/[^\/]+)$/gi, "https://gist.githubusercontent.com/$1/raw/");</span>
  }
...
export function resolveUri(baseUri: string, pathOrUri: string): string {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (pathOrUri.startsWith("~/")) {
<span class="cstat-no" title="statement not covered" >    pathOrUri = pathOrUri.replace(/^~/, homedir());</span>
  }
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!baseUri) {
<span class="cstat-no" title="statement not covered" >    throw new Error("'pathOrUri' was detected to be relative so 'baseUri' is required");</span>
  }
  try {
    const base = new URI(baseUri);
    const relative = new URI(pathOrUri);
    <span class="missing-if-branch" title="if path not taken" >I</span>if (baseUri.startsWith("untitled:///") &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >pathOrUri.startsWith("untitled:"))</span> {
<span class="cstat-no" title="statement not covered" >      return pathOrUri;</span>
    }
...
<span class="cstat-no" title="statement not covered" >    throw new Error(`Failed resolving '${pathOrUri}' against '${baseUri}'.`);</span>
  }
}
&nbsp;
...
&nbsp;
export function <span class="fstat-no" title="function not covered" >makeRelativeUri(</span>baseUri: string, absoluteUri: string): string {
<span class="cstat-no" title="statement not covered" >  return new URI(absoluteUri).relativeTo(baseUri).toString();</span>
}
//------------------------------------------
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Fri Mar 19 2021 22:40:57 GMT+0000 (Coordinated Universal Time)
            </div>
        </div>
