...
import { getEffectiveModelType, ignoreDiagnostics, isNeverType, isUnknownType, isVoidType, } from "@typespec/compiler";
...
import { extractLroStates, findLroStatusProperty, } from "./decorators/lro-status.js";
import { getOperationLink, getOperationLinks, } from "./decorators/operation-link.js";
import { getPollingLocationInfo, isPollingLocation, pollingOptionsKind, } from "./decorators/polling-location.js";
...
export function getLroMetadata(program, operation) {
    ...
    if (nextReference !== undefined && nextReference.responseModel.kind === "Model") {
        ...
        const linkedOperation = nextReference.kind === "nextOperationLink"
...
function createLroMetadata(program, operation, context) {
    ...
    const logicalPathName = context.finalStep?.kind === "pollingSuccessProperty"
        ...
        : undefined;
    let finalResult = model.kind === "Model" || (model.kind === "Intrinsic" && !isVoidType(model)) ? model : "void";
    let finalEnvelopeResult = model.kind === "Model" || (model.kind === "Intrinsic" && !isVoidType(model)) ? model : "void";
...
function filterSuccessType(program, type) {
    if (type.kind === "Model")
        return type;
    if (isNeverType(type))
