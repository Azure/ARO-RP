import { getNamespaceFullName, getTypeName, isTemplateDeclaration, isTemplateInstance, isType, navigateProgram, } from "@typespec/compiler";
import { $added, $removed, findVersionedNamespace, getMadeOptionalOn, getMadeRequiredOn, getRenamedFrom, getReturnTypeChangedFrom, getTypeChangedFrom, getUseDependencies, } from "./decorators.js";
import { reportDiagnostic } from "./lib.js";
import { getVersionAdditionCodefixes, getVersionRemovalCodeFixes } from "./validate.codefix.js";
import { Availability, getAllVersions, getAvailabilityMap, getVersionDependencies, getVersions, } from "./versioning.js";
const relationCacheKey = Symbol.for("TypeSpec.Versioning.NamespaceRelationCache");
export function getCachedNamespaceDependencies(program) {
...
export function $onValidate(program) {
    ...
    function addNamespaceDependency(source, target) {
        ...
        const set = namespaceDependencies.get(source) ?? new Set();
...
function getVersionedNameMap(program, source) {
    ...
    if (allVersions === undefined)
        return undefined;
    const map = new Map(allVersions.map((v) => [v, undefined]));
 ...
 */
function validateVersionEnumValuesUnique(program, namespace) {
...
function validateTargetVersionCompatible(program, source, target, validateOptions = {}) {
    ...
    if (validateOptions.isTargetADependent) {
        ...
        validateAvailabilityForRef(program, sourceAvailability.map, targetAvailability.map, sourceAvailability.type, targetAvailability.type, versionMap instanceof Map ? versionMap : undefined);
    }
}
