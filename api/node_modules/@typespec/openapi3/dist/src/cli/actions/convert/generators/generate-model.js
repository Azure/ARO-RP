...
import { getTypeSpecPrimitiveFromSchema, isReferencedEnumType, isReferencedUnionType, SchemaToExpressionGenerator, } from "./generate-types.js";
...
function generateSSEEventVariants(members, union, context, getVariantName) {
    ...
    return sortedMembers.map((member) => {
        try {
            ...
            if (!memberSchema || typeof memberSchema !== "object" || !memberSchema.properties) {
                ...
                return (getVariantName(member) + context.generateTypeFromRefableSchema(member, union.scope) + ",");
            }
...
function generateUnion(union, context) {
    ...
    if (schema.enum) {
        ...
        const isSSEEventUnion = union.decorators.some((d) => d.name === "TypeSpec.Events.events" || d.name === "events");
        if (isSSEEventUnion) {
            definitions.push(...generateSSEEventVariants(schema.oneOf, union, context, getVariantName));
        }
        else {
            // Regular union generation
            definitions.push(...schema.oneOf.map((member) => getVariantName(member) +
                context.generateTypeFromRefableSchema(member, union.scope) +
                ","));
        }
    }
    else if (schema.anyOf) {
        definitions.push(...schema.anyOf.map((member) => getVariantName(member) + context.generateTypeFromRefableSchema(member, union.scope) + ","));
    }
...
function generateModel(model, context) {
    ...
    definitions.push(...model.properties.map((prop) => generateModelProperty(prop, model.scope, context, model.isModelReferencedAsMultipartRequestBody, model.encoding)));
...
export function generateModelProperty(prop, containerScope, context, isModelReferencedAsMultipartRequestBody, encoding) {
    ...
    const decorators = generateDecorators([...prop.decorators, ...getDecoratorsForSchema(prop.schema)], isModelReferencedAsMultipartRequestBody
