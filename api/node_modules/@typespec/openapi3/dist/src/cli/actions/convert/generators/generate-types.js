...
import { getDecoratorsForSchema, normalizeObjectValueToTSValueExpression, } from "../utils/decorators.js";
...
export class SchemaToExpressionGenerator {
    ...
    getTypeFromSchema(schema, callingScope, isHttpPart = false, encoding, context) {
        let type = "unknown";
    ...
    generateDefaultValue(schema, callingScope, context) {
    ...
    getAllOfType(schema, callingScope) {
        ...
        for (const name of Object.keys(properties)) {
            ...
            props.push(`${printIdentifier(name)}${isOptional}: ${this.generateTypeFromRefableSchema(properties[name], callingScope)}`);
        }
    ...
    stripDefaultsFromSchema(schema) {
        if ("$ref" in schema) {
            return schema;
        }
        ...
        if (strippedSchema.additionalProperties &&
    ...
    getPartType(propType, name, isHttpPart, encoding, isEnumType, isUnionType) {
        ...
        const filePartType = encodingForProperty?.contentType &&
            ...
            : undefined;
        const contentTypeHeader = encodingForProperty?.contentType &&
            !filePartType &&
    ...
    isInlineUnionType(partType) {
        return partType.includes("|");
    }
    isScalarType(partType) {
        return (partType === "string" ||
            partType === "boolean" ||
            partType === "null" ||
            this.numericTypes[partType]);
    }
    isDefaultPartType(partType, partMediaType) {
        return (((partType === "string" || this.numericTypes[partType]) && partMediaType === "text/plain") ||
            (partType === "bytes" && partMediaType === "application/octet-stream"));
    }
    shouldUpgradeToFileDefinition(partType, partMediaType) {
    ...
    static decoratorNamesToExcludeForParts = ["minValue", "maxValue"];
    getObjectType(schema, callingScope, isHttpPart = false, encoding, context) {
        // If we have `additionalProperties`, treat that as an 'indexer' and convert to a record.
        const recordType = schema.additionalProperties === true
            ? "Record<unknown>"
        ...
        if (schema.properties) {
            for (const name of Object.keys(schema.properties)) {
                ...
                const decorators = generateDecorators(getDecoratorsForSchema(originalPropSchema), isHttpPart ? SchemaToExpressionGenerator.decoratorNamesToExcludeForParts : [])
                    .map((d) => `${d}\n`)
                    .join("");
                const isOptional = !requiredProps.includes(name) ? "?" : "";
                props.push(`${decorators}${printIdentifier(name)}${isOptional}: ${this.getPartType(propType, name, isHttpPart, encoding, isEnumType, isUnionType)}`);
            }
        }
...
export function isReferencedEnumType(propSchema, context) {
...
export function isReferencedUnionType(propSchema, context) {
    let isUnionType = false;
    try {
        const resolvedSchema = "$ref" in propSchema ? context.getSchemaByRef(propSchema.$ref) : undefined;
        const resolvedItemsSchema = "items" in propSchema && propSchema.items && "$ref" in propSchema.items
