...
export function transform(context) {
    ...
    if (!context.openApi3Doc.openapi.startsWith("3.0") &&
 ...
 */
function scanForMultipartSchemas(openapi, context) {
    if (!openapi.paths)
        return;
 ...
 */
function scanForSSESchemas(openapi, context) {
    if (!openapi.paths)
        return;
    for (const path of Object.values(openapi.paths)) {
        if (!path)
...
function scanPathForMultipartSchemas(path, context) {
...
function scanOperationForMultipartSchemas(requestBodyRef, context) {
    const requestBody = resolveReference(requestBodyRef, context);
    if (!requestBody)
        return;
    if (!("content" in requestBody) || !requestBody.content)
        return;
    for (const [_, contentBody] of Object.entries(requestBody.content).filter(([mediaType, _]) => mediaType === "multipart/form-data")) {
...
function scanPathForSSESchemas(path, context) {
    for (const method of methods) {
        const operation = path[method];
...
function scanOperationForSSESchemas(responses, context) {
