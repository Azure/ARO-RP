...
import { createCLICompilerHost, handleInternalCompilerError, logDiagnosticCount, withCliHost, withCliHostAndDiagnostics, } from "./utils.js";
/**
 * Intercept `tsp compile --emit <emitter> --help` to display emitter options.
 * Returns true if the interception was handled.
 */
async function handleCompileEmitHelp(argv) {
...
async function main() {
    ...
    await yargs(argv)
        ...
        return cmd
            ...
            .command("install", "Install VS Code Extension", () => { }, withCliHostAndDiagnostics((host, args) => installVSCodeExtension(host, args)))
            .command("uninstall", "Uninstall VS Code Extension", () => { }, withCliHostAndDiagnostics((host, args) => uninstallVSCodeExtension(host, args)));
    })
        .command("vs", "Manage Visual Studio Extension.", (cmd) => {
        return cmd
            .demandCommand(1, "No command specified")
            .command("install", "Install Visual Studio Extension.", () => { }, withCliHostAndDiagnostics((host) => installVSExtension(host)))
            .command("uninstall", "Uninstall VS Extension", () => { }, withCliHostAndDiagnostics((host) => uninstallVSExtension(host)));
    })
        ...
        .command("info [emitter]", "Show information about the current TypeSpec compiler, or about a specific emitter.", (cmd) => {
