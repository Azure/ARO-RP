...
export function createResolver(program) {
    ...
    function resolveTypeReference(node, options = {}) {
        ...
        else if (resolvedSym && resolvedSym.flags & 1024 /* SymbolFlags.TemplateParameter */) {
    ...
    function resolveMemberExpressionForSym(baseSym, node, options = {}) {
        if (node.selector === ".") {
            ...
            else if (baseSym.flags & 33024 /* SymbolFlags.ExportContainer */) {
    ...
    function resolveReturnType(node) {
        ...
        if (baseSym.flags & 2048 /* SymbolFlags.Function */) {
    ...
    function bindDeclarationIdentifier(node) {
        ...
        if (node.symbol.flags & 65536 /* SymbolFlags.Member */) {
    ...
    function resolveIdentifier(node, options) {
        ...
        while (scope && scope.kind !== SyntaxKind.TypeSpecScript) {
            if (scope.symbol && scope.symbol.flags & 33024 /* SymbolFlags.ExportContainer */) {
            ...
            if ("locals" in scope && scope.locals !== undefined) {
                ...
                if (binding) {
                    if (binding.flags & 8192 /* SymbolFlags.Using */ && binding.symbolSource) {
        ...
        if (!binding && scope && scope.kind === SyntaxKind.TypeSpecScript) {
            ...
            if (globalBinding && usingBinding) {
                ...
                if (usingBinding.flags & 16384 /* SymbolFlags.DuplicateUsing */) {
                    return ambiguousResult([
                        ...(augmentedSymbolTables.get(scope.locals)?.duplicates.get(usingBinding) ??
                            []),
                    ]);
                }
                if (usingBinding.flags & 8192 /* SymbolFlags.Using */ && usingBinding.symbolSource) {
    ...
    function mergeSymbolTable(source, target) {
        ...
        for (const [key, sourceBinding] of source) {
            ...
            else if (sourceBinding.flags & 512 /* SymbolFlags.Decorator */) {
                mergeDeclarationOrImplementation(key, sourceBinding, target, 512 /* SymbolFlags.Decorator */);
            }
            else if (sourceBinding.flags & 2048 /* SymbolFlags.Function */) {
                mergeDeclarationOrImplementation(key, sourceBinding, target, 2048 /* SymbolFlags.Function */);
            }
            else {
                target.set(key, sourceBinding);
            }
