...
async function addIdentifierCompletion({ program, completions }, node) {
    ...
    for (const [key, { sym, label, suffix }] of result) {
        ...
        if (sym.flags & (2048 /* SymbolFlags.Function */ | 512 /* SymbolFlags.Decorator */)) {
            kind = CompletionItemKind.Function;
        }
        else if (sym.flags & 256 /* SymbolFlags.Namespace */ &&
        ...
        if (sym.name.startsWith("$")) {
            ...
            Range.create(lineAndChar, lineAndChar), printIdentifier(key) + (suffix ?? ""));
        }
        else {
    ...
    function canAccessCompletionSymbol(sym, sourceLocation) {
        const isInternalDeclaration = (sym.flags & (8388608 /* SymbolFlags.Internal */ | 1048576 /* SymbolFlags.Declaration */)) ===
