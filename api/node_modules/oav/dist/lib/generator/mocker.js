...
class Mocker {
    mock(paramSpec, paramName, arrItem) {
        switch (paramSpec.type) {
            ...
            default:
                logging_1.log.warn(`unknown type ${paramSpec.type}.`);
        }
    }
    generateString(paramSpec, paramName) {
    ...
    getRandomInt(min, max, minInclusive = true, maxInclusive = true) {
        min = Math.ceil(min);
        max = Math.floor(max);
        if (max === min) {
            return min;
    ...
    generateBoolean(_paramSpec) {
        return true;
    }
    generateArray(paramSpec, paramName, arrItem) {
        if (!arrItem) {
            logging_1.log.warn(`array ${paramName} item is null, it may be caused by circular reference`);
            return [];
        }
        ...
        if (uniqueItems) {
            if (minItems > 1) {
                console.error(`array ${paramName} can not be mocked with both uniqueItems=true and minItems=${minItems}`);
                return [];
            }
            return [arrItem];
        }
