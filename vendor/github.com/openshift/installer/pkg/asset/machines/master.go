...
func (m *Master) Generate(dependencies asset.Parents) error {
	...
	if dnsmasqEnabled {
		...
		ignARODNS, err = dnsv2.MachineConfig(installConfig.Config.ClusterDomain(), aroDNSConfig.APIIntIP, aroDNSConfig.IngressIP, "master", aroDNSConfig.GatewayDomains, aroDNSConfig.GatewayPrivateEndpointIP)
	}
	if err != nil {
