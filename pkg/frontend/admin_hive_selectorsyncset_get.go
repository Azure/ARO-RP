...
func (f *frontend) getAdminHiveSelectorSyncSet(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	log := ctx.Value(middleware.ContextKeyLog).(*logrus.Entry)
	resourceID := strings.TrimPrefix(filepath.Dir(r.URL.Path), "/admin")
	b, err := f._getAdminHiveSelectorSyncSet(ctx, resourceID)
...
func (f *frontend) _getAdminHiveSelectorSyncSet(ctx context.Context, resourceID string) ([]byte, error) {
	...
	cd, err := f.hiveSyncSetManager.GetSelectorSyncSet(ctx, doc)
	if err != nil {
