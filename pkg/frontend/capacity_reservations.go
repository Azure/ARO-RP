...
func crgSetupForResize(ctx context.Context, log *logrus.Entry, a crgActions, clusterRG, location string, zones []string, targetSKU, crgName string) (bool, error) {
	log.Infof("setting up capacity reservation group %s for SKU %s in zones %v", crgName, targetSKU, zones)
	if _, err := a.CreateCRG(ctx, clusterRG, location, zones, crgName); err != nil {
...
func crgTeardown(ctx context.Context, log *logrus.Entry, a crgActions, clusterRG, crgName string, zones []string) error {
