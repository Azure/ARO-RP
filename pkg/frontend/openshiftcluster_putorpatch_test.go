...
func TestPutOrPatchOpenShiftClusterAdminAPI(t *testing.T) {
	...
	for _, tt := range []*test{
		...
		{
			name: "patch an empty maintenane state cluster with maintenance none request",
			request: func(oc *admin.OpenShiftCluster) {
		...
		{
			name: "patch an planned ongoing cluster with maintenance none request",
			request: func(oc *admin.OpenShiftCluster) {
				oc.Properties.MaintenanceTask = admin.MaintenanceTaskNone
			},
			isPatch: true,
		...
		{
			name: "patch an unplanned ongoing cluster with maintenance none request",
			request: func(oc *admin.OpenShiftCluster) {
				oc.Properties.MaintenanceTask = admin.MaintenanceTaskNone
			},
			isPatch: true,
