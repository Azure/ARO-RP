...
func TestAdminApproveCSR(t *testing.T) {
	...
	for _, tt := range []*test{
		...
		t.Run(fmt.Sprintf("%s: %s", tt.method, tt.name), func(t *testing.T) {
			...
			f, err := NewFrontend(ctx, ti.audit, ti.otelAudit, ti.log, ti.env, ti.dbGroup, api.APIs, &noop.Noop{}, &noop.Noop{}, nil, nil, func(*logrus.Entry, env.Interface, *api.OpenShiftCluster) (adminactions.KubeActions, error) {
				return k, nil
			}, nil, nil, nil)
