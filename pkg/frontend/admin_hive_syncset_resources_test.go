	...
	for _, tt := range []*test{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.hiveEnabled {
				...
				clusterManager.EXPECT().GetSyncSetResources(gomock.Any(), gomock.Any()).Return(&clusterSync, nil).Times(tt.expectedGetClusterSyncCallCount)
			...
			if !strings.EqualFold(string(hiveClusterSync), string(tt.wantResponse)) {
				t.Fatalf("got %q and expected %q", hiveClusterSync, tt.wantResponse)
			}
		})
	}
}
