...
func (f *frontend) _getAdminHiveClusterSync(ctx context.Context, resourceID string) ([]byte, error) {
	...
	cs, err := f.hiveClusterManager.GetClusterSync(ctx, doc.OpenShiftCluster)
	if err != nil {
		return nil, api.NewCloudError(http.StatusNotFound, api.CloudErrorCodeNotFound, "", "cluster sync not found")
	}

	var b []byte
	err = codec.NewEncoderBytes(&b, &codec.JsonHandle{}).Encode(cs)
	if err != nil {
