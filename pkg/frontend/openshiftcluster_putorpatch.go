...
func (f *frontend) _putOrPatchOpenShiftCluster(ctx context.Context, log *logrus.Entry, putOrPatchClusterParameters PutOrPatchOpenshiftClusterParameters) ([]byte, error) {
	...
	case http.MethodPatch:
		...
		if doc.OpenShiftCluster.UsesWorkloadIdentity() {
			tempPatchParameters := &admin.OpenShiftCluster{}
			...
			if tempPatchParameters.Identity != nil && len(tempPatchParameters.Identity.UserAssignedIdentities) > 0 {
				for k := range doc.OpenShiftCluster.Identity.UserAssignedIdentities {
					delete(doc.OpenShiftCluster.Identity.UserAssignedIdentities, k)
				}
			}
		}
