	...
	for _, tt := range []*test{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.hiveEnabled {
				...
				f, err = NewFrontend(ctx, ti.audit, ti.otelAudit, ti.log, _env, ti.dbGroup, api.APIs, &noop.Noop{}, &noop.Noop{}, nil, clusterManager, nil, nil, nil, nil)
			} else {
				f, err = NewFrontend(ctx, ti.audit, ti.otelAudit, ti.log, _env, ti.dbGroup, api.APIs, &noop.Noop{}, &noop.Noop{}, nil, nil, nil, nil, nil, nil)
			}
