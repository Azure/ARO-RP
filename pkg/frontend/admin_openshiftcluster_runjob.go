...
func parseAndValidateJob(body []byte) (*batchv1.Job, error) {
	...
	// API server rejects creates when these are set
	job.UID = ""
	job.ResourceVersion = ""
	...
	// 57 + 1 dash + 5 random = 63-char DNS label limit
...
func runJobStream(ctx context.Context, log *logrus.Entry, k adminactions.KubeActions, job *batchv1.Job, w io.WriteCloser, delay time.Duration) {
	...
	if err := k.KubeCreateOrUpdate(ctx, un); err != nil {
		...
		// no cleanup: a failed create leaves no resource; stale jobs are GC'd by Kubernetes TTL
		return
	}
	...
	if err != nil {
		// first ctx.Err() selects log level; second suppresses output; cleanupJob runs regardless
		if ctx.Err() != nil {
	...
	// poll for terminal state in case log streaming ended early (e.g. truncation)
...
func waitForJobPod(ctx context.Context, log *logrus.Entry, watcher watch.Interface) (string, error) {
	for {
		...
		case event, ok := <-watcher.ResultChan():
			...
			// PodPending not surfaced; stuck-pending pods are bounded by the caller's opCtx timeout
...
func jobResult(ctx context.Context, k adminactions.KubeActions, namespace, jobName string) (string, error) {
	...
	for _, cond := range job.Status.Conditions {
		...
		// JobSuspended and FailureTarget fall through to "pending"; backoffLimit=0 makes FailureTarget unreachable
		switch cond.Type {
...
// uses context.Background() so request cancellation cannot prevent cleanup.
