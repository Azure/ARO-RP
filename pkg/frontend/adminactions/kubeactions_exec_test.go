...
// surfaces the metav1.Status.Message from the response body when available, falls back to the raw
// HTTP status and body for non-Status responses, and keeps the "WebSocket upgrade:" prefix in all
// cases so callers can recognise upgrade failures consistently.
func TestDialExecWebSocket_UpgradeRejected(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			name:    "kubernetes status message surfaced",
			body:    `{"kind":"Status","apiVersion":"v1","metadata":{},"status":"Failure","message":"container etcdctl is not running in pod openshift-etcd/etcd-master-0","reason":"BadRequest","code":400}`,
			wantErr: "container etcdctl is not running in pod openshift-etcd/etcd-master-0",
		},
		{
			name:    "non-JSON body surfaces HTTP status and body",
			body:    "not json",
			wantErr: "WebSocket upgrade:",
		},
		{
			// Bodies larger than recorderBufSize are captured partially; the partial body is
			// truncated to 256 bytes and surfaced alongside the HTTP status code.
			name:    "large body surfaces HTTP status and truncated body",
			body:    `{"kind":"Status","apiVersion":"v1","metadata":{},"status":"Failure","message":"` + strings.Repeat("x", 5000) + `","reason":"BadRequest","code":400}`,
			wantErr: "WebSocket upgrade:",
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			execURL, _ := url.Parse(ts.URL)
			execURL.Path = "/exec"
			execURL.Scheme = "https"

			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()

			_, _, err := dialExecWebSocket(ctx, rc, execURL)
			if err == nil {
