...
func TestOpenShiftPodsTop(t *testing.T) {
	...
	for _, tt := range []*test{
		...
		t.Run(fmt.Sprintf("%s: %s", tt.method, tt.name), func(t *testing.T) {
			...
			resp, b, err := ti.request(tt.method,
				fmt.Sprintf("https://server%s/toppods?api-version=2020-04-30&namespace=%s", coalesce(tt.resourceIDOverride, tt.resourceID), tt.objNamespace),
				nil, nil)
			if err != nil {
				t.Fatal(err)
			}
