...
func TestDeleteClusterManagerConfiguration(t *testing.T) {
	...
	for _, tt := range []*test{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			f, err := NewFrontend(ctx, ti.audit, ti.log, ti.env, nil, ti.clusterManagerDatabase, nil, ti.subscriptionsDatabase, nil, api.APIs, &noop.Noop{}, &noop.Noop{}, nil, nil, nil, nil, nil, nil)
			if err != nil {
				t.Fatal(err)
			}
