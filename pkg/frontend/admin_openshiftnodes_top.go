...
func (f *frontend) topAdminOpenShiftNodes(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	log := ctx.Value(middleware.ContextKeyLog).(*logrus.Entry)
	r.URL.Path = filepath.Dir(r.URL.Path)

	b, err := f._topAdminOpenShiftNodes(ctx, r, log)

	adminReply(log, w, nil, b, err)
}

func (f *frontend) _topAdminOpenShiftNodes(ctx context.Context, r *http.Request, log *logrus.Entry) ([]byte, error) {
	vars := mux.Vars(r)
