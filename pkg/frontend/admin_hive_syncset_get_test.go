	...
	for _, tt := range []*test{
		{
			...
			wantStatusCode: 403,
			wantError:      "403: Forbidden: : namespace should be null for getting a selectorsyncset",
		},
		{
			...
			mocks:          func(tt *test, s *mock_hive.MockSyncSetManager) {},
			wantStatusCode: 403,
			wantError:      "403: Forbidden: : namespace cannot be null for getting a syncset",
		},
		{
			...
			mocks:          func(tt *test, s *mock_hive.MockSyncSetManager) {},
			wantStatusCode: 403,
			wantError:      "403: Forbidden: : syncsetname cannot be null",
		},
		{
			...
			wantStatusCode: 200,
			wantResponse:   []byte(`{"name":"selectorsyncset-1","namespace":""}`),
			wantError:      "",
		},
		{
			name:        "get a syncset",
			...
			wantStatusCode: 200,
			wantResponse:   []byte(`{"name":"syncset-1","namespace":"aro-00000000-0000-0000-0000-000000000000"}`),
			wantError:      "",
		},
	} {
