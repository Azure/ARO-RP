		...
		t.Run(tt.name, func(t *testing.T) {
			for name, listPrefix := range map[string]string{
				...
				t.Run(name, func(t *testing.T) {
					...
					f, err := NewFrontend(ctx, ti.audit, ti.log, ti.env, ti.asyncOperationsDatabase, ti.clusterManagerDatabase, ti.openShiftClustersDatabase, ti.subscriptionsDatabase, nil, nil, api.APIs, &noop.Noop{}, &noop.Noop{}, aead, nil, nil, nil, nil, ti.enricher)
					if err != nil {
						t.Fatal(err)
					}
