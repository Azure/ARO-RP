...
// allowPrivilegeEscalation and allowedCapabilities omitted: the recovery pod only needs hostDir access.
...
// returns a cleanup function to tear down SA, ClusterRole, CRB, and SCC.
func createPrivilegedServiceAccount(ctx context.Context, log *logrus.Entry, name, roleName string, kubeActions adminactions.KubeActions) (func() error, error) {
	...
	// uses context.Background() so request cancellation cannot prevent RBAC/SCC teardown
	cleanup := func() error {
