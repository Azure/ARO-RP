...
func (f *frontend) _listAdminHiveSyncSet(ctx context.Context, namespace string, label string, isSyncSet bool) ([]byte, error) {
	...
	if isSyncSet && namespace == "" {
		return nil, api.NewCloudError(http.StatusForbidden, api.CloudErrorCodeForbidden, "", "namespace cannot be null for listing syncsets")
	}
	if !isSyncSet && namespace != "" {
		return nil, api.NewCloudError(http.StatusForbidden, api.CloudErrorCodeForbidden, "", "namespace should be null for listing selectorsyncsets")
	}
