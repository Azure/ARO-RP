...
func (f *frontend) topOpenShiftPods(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	log := ctx.Value(middleware.ContextKeyLog).(*logrus.Entry)
	r.URL.Path = filepath.Dir(r.URL.Path)

	b, err := f._topOpenShiftPods(ctx, r, log)

	reply(log, w, nil, b, err)
}

func (f *frontend) _topOpenShiftPods(ctx context.Context, r *http.Request, log *logrus.Entry) ([]byte, error) {
	vars := mux.Vars(r)
