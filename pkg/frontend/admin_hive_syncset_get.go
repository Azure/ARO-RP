...
func (f *frontend) _getAdminHiveSyncSet(ctx context.Context, namespace string, syncsetname string, isSyncSet bool) ([]byte, error) {
	...
	if isSyncSet && namespace == "" {
		return nil, api.NewCloudError(http.StatusForbidden, api.CloudErrorCodeForbidden, "", "namespace cannot be null for getting a syncset")
	}
	if !isSyncSet && namespace != "" {
		return nil, api.NewCloudError(http.StatusForbidden, api.CloudErrorCodeForbidden, "", "namespace should be null for getting a selectorsyncset")
	}
	if syncsetname == "" {
		return nil, api.NewCloudError(http.StatusForbidden, api.CloudErrorCodeForbidden, "", "syncsetname cannot be null")
	}

	log.Printf("Calling Get with context: %v, namespace: %s, name: %s, type: %v", ctx, namespace, syncsetname, ssType)
