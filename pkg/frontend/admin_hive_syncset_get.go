...
func (f *frontend) getAdminHiveSyncSet(w http.ResponseWriter, r *http.Request) {
	...
	b, err := f._getAdminHiveSyncSet(ctx, resourceID)
...
func (f *frontend) _getAdminHiveSyncSet(ctx context.Context, resourceID string) ([]byte, error) {
	...
	cd, err := f.hiveSyncSetManager.GetSyncSet(ctx, doc)
	if err != nil {
		return nil, api.NewCloudError(http.StatusNotFound, api.CloudErrorCodeNotFound, "", "syncset not found")
	}
