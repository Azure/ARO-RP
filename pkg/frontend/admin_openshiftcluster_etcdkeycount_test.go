	...
	for _, tt := range []*test{
		{
			...
			mocks: func(tt *test, k *mock_adminactions.MockKubeActions) {
				k.EXPECT().
					KubeExecStream(gomock.Any(), namespaceEtcds, "etcd-master-0", etcdKeyCountContainer,
						[]string{"sh", "-c", etcdKeyCountCommand}, gomock.Any(), gomock.Any()).
		...
		t.Run(fmt.Sprintf("%s: %s", method, tt.name), func(t *testing.T) {
			...
			u := fmt.Sprintf("https://server/admin%s/etcdkeycount?nodeName=%s", resourceID, url.QueryEscape(tt.nodeName))
