...
func (f *frontend) listAdminHiveSelectorSyncSet(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	log := ctx.Value(middleware.ContextKeyLog).(*logrus.Entry)
	resourceID := strings.TrimPrefix(filepath.Dir(r.URL.Path), "/admin")
	b, err := f._listAdminHiveSelectorSyncSet(ctx, resourceID)
...
func (f *frontend) _listAdminHiveSelectorSyncSet(ctx context.Context, resourceID string) ([]byte, error) {
	...
	cd, err := f.hiveSyncSetManager.ListSelectorSyncSets(ctx, doc)
	if err != nil {
		return nil, api.NewCloudError(http.StatusNotFound, api.CloudErrorCodeNotFound, "", "Selector syncset not found")
	}
