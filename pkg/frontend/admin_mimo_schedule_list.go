...
func (f *frontend) getAdminMaintSchedules(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	log := ctx.Value(middleware.ContextKeyLog).(*logrus.Entry)
	b, err := f._getAdminMaintSchedules(ctx, r)
...
func (f *frontend) _getAdminMaintSchedules(ctx context.Context, r *http.Request) ([]byte, error) {
	...
	i := dbMaintenanceSchedules.List(skipToken)

	docList := make([]*api.MaintenanceScheduleDocument, 0)
	for {
		...
		docList = append(docList, docs.MaintenanceScheduleDocuments...)
