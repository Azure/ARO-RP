...
func TestEnsureBootstrapLBConfig(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := m.ensureBootstrapNodeLBConfig(&tt.lb)
...
func TestILBName(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			name:        "V1 returns internal-lb suffix",
			archVersion: api.ArchitectureVersionV1,
			wantName:    bsTestInfraID + "-internal-lb",
		},
		{
			name:        "V2 returns internal suffix",
			archVersion: api.ArchitectureVersionV2,
			wantName:    bsTestInfraID + "-internal",
		},
		{
			name:        "unknown version returns error",
			archVersion: api.ArchitectureVersion(99),
			wantErr:     "unknown architecture version 99",
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			_, log := testlog.New()
			m := &manager{
				...
				doc: &api.OpenShiftClusterDocument{
					OpenShiftCluster: &api.OpenShiftCluster{
						Properties: api.OpenShiftClusterProperties{
							ArchitectureVersion: tt.archVersion,
						},
					},
				},
			}

			got, err := m.ilbName(bsTestInfraID)
