...
func TestLogLoadBalancers(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:        "unknown architecture version returns error",
			doc:         newLBTestDoc(lbTestResourceGroupID, lbTestInfraID, api.ArchitectureVersion(99)),
			mockLB:      func(m *mock_armnetwork.MockLoadBalancersClient) {},
			mockMetrics: func(m *mock_azmetrics.MockMetricsClient) {},
			wantErr:     "unknown architecture version 99",
		},
		{
			name: "ArchitectureVersionV1 uses internal-lb suffix",
			doc:  newLBTestDoc(lbTestResourceGroupID, lbTestInfraID, api.ArchitectureVersionV1),
			mockLB: func(m *mock_armnetwork.MockLoadBalancersClient) {
				m.EXPECT().Get(gomock.Any(), lbTestResourceGroup, lbTestInfraID+"-internal-lb", nil).
