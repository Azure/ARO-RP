...
func (m *manager) generatePlatformWorkloadIdentitySecretsAndNamespaces(isCreate bool) ([]*corev1.Secret, []*corev1.Namespace, error) {
	...
	rolesByRoleName := m.platformWorkloadIdentityRolesByVersion.GetPlatformWorkloadIdentityRolesByRoleName()
	...
	for name, identity := range m.doc.OpenShiftCluster.Properties.PlatformWorkloadIdentityProfile.PlatformWorkloadIdentities {
		roles, exists := rolesByRoleName[name]
		if !exists {
			return nil, nil, platformworkloadidentity.GetPlatformWorkloadIdentityMismatchError(m.doc.OpenShiftCluster, rolesByRoleName)
		}
		...
		if name == pkgoperator.OperatorIdentityName && isCreate {
			continue
		}
...
func (m *manager) ensurePlatformWorkloadIdentityRBAC(ctx context.Context) error {
	...
	roleAssignmentsForManagedResourceGroupByPrincipalID := map[string]map[string]mgmtauthorization.RoleAssignment{}
	for _, roleAssignment := range allExistingRoleAssignments {
		if strings.EqualFold(*roleAssignment.Scope, resourceGroupID) {
			if _, ok := roleAssignmentsForManagedResourceGroupByPrincipalID[strings.ToLower(*roleAssignment.PrincipalID)]; !ok {
				roleAssignmentsForManagedResourceGroupByPrincipalID[strings.ToLower(*roleAssignment.PrincipalID)] = map[string]mgmtauthorization.RoleAssignment{}
			}
			roleAssignmentsForManagedResourceGroupByPrincipalID[strings.ToLower(*roleAssignment.PrincipalID)][strings.ToLower(*roleAssignment.RoleDefinitionID)] = roleAssignment
		}
	}
