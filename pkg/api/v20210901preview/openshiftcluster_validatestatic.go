...
func (sv openShiftClusterStaticValidator) validateNetworkProfile(path string, np *NetworkProfile) error {
	...
	default:
		return api.NewCloudError(http.StatusBadRequest, api.CloudErrorCodeInvalidParameter, path+".SoftwareDefinedNetwork", fmt.Sprintf("The provided SoftwareDefinedNetwork '%s' is invalid.", np.SoftwareDefinedNetwork))
	}
