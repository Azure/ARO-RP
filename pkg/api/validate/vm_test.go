...
func TestVMSizeIsValid(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			name:          "CI mode: Standard_D2s_v4 is valid as worker",
			vmSize:        vms.VMSizeStandardD2sV4,
			isMaster:      false,
			isCI:          true,
			desiredResult: true,
		},
		{
			name:          "CI mode: Standard_D2s_v5 is valid as worker",
		...
		{
			name:          "CI mode: Standard_F72s_v2 is valid as master",
			vmSize:        vms.VMSizeStandardF72sV2,
			isMaster:      true,
			isCI:          true,
			desiredResult: true,
		...
		{
			name:          "CI mode: Standard_D4s_v3 is valid as master",
