...
func newCertificateRefresher(ctx context.Context, logger *logrus.Entry, interval time.Duration, kv keyvault.Manager, certificateName string, stop <-chan struct{}) CertificateRefresher {
	r := &refreshingCertificate{
		logger:   logger,
		interval: interval,
		kv:       kv,
...
// GetCertificates loads the certificate from the atomic store safe to use concurently
