...
func TestSecurity(t *testing.T) {
	...
	p := NewPortal(_env, portalAuditLog, otelAudit, portalLog, portalAccessLog, l, sshl, nil, "", serverkey, servercerts, "", nil, nil, make([]byte, 32), sshkey, nil, elevatedGroupIDs, dbg, nil, &noop.Noop{})
	go func() {
		err := p.Run(ctx)
		if err != nil {
			log.Error(err)
