...
func (cmc *clusterManagerConfigurationBackend) try(ctx context.Context, backendDoc *api.BackendDocument) (bool, error) {
	...
	if docs != nil {
		log.Info("CMC: ", len(docs.ClusterManagerConfigurationDocuments), " documents in change feed")
		for _, doc := range docs.ClusterManagerConfigurationDocuments {
			config := doc.ClusterManagerConfiguration
			object, gvk, err := cmc.jsonSerializer.Decode(config.Properties.Resources, nil, nil)
			if err != nil {
				log.Errorf("Error decoding resource ID %q: %s", config.ID, err)
			} else if config.Deleting {
