...
func (ocb *openShiftClusterBackend) handle(ctx context.Context, log *logrus.Entry, doc *api.OpenShiftClusterDocument) error {
	...
	m, err := openshiftcluster.NewManager(log, ocb.env, ocb.fp, ocb.gl, ocb.dialer, ocb.fakearm, ocb.dbopenshiftclusters, ocb.cipher, ocb.billing, doc, subscriptionDoc, ocb.clustersKeyvaultURI)
	if err != nil {
