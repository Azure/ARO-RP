...
func (ocb *openShiftClusterBackend) handle(ctx context.Context, log *logrus.Entry, doc *api.OpenShiftClusterDocument) error {
	...
	case api.ProvisioningStateAdminUpdating:
		...
		if err != nil {
			return ocb.endLease(ctx, log, stop, doc, doc.OpenShiftCluster.Properties.LastProvisioningState, err)
		}
		return ocb.endLease(ctx, log, stop, doc, doc.OpenShiftCluster.Properties.LastProvisioningState, nil)
