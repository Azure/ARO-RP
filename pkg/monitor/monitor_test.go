...
func fakeClusterMonitorBuilder(log *logrus.Entry, restConfig *rest.Config, oc *api.OpenShiftCluster, env env.Interface, tenantID string, m metrics.Emitter, hourlyRun bool, wg *sync.WaitGroup) (monitoring.Monitor, error) {
	return &fakeMonitor{
		Wg:             wg,
...
func fakeHiveMonitoringBuilder(log *logrus.Entry, oc *api.OpenShiftCluster, m metrics.Emitter, hourlyRun bool, wg *sync.WaitGroup, hiveClusterManager hive.ClusterManager) (monitoring.Monitor, error) {
	return &monitoring.NoOpMonitor{
		Wg: wg,
	}, nil
}

func fakeNsgMonitoringBuilder(log *logrus.Entry, oc *api.OpenShiftCluster, e env.Interface, subscriptionID, tenantID string, emitter metrics.Emitter, dims map[string]string, wg *sync.WaitGroup, trigger <-chan time.Time) monitoring.Monitor {
	return &monitoring.NoOpMonitor{
		Wg: wg,
	}
}
