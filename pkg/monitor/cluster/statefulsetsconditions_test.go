...
func TestEmitStatefulSetsConditions(t *testing.T) {
	...
	m.EXPECT().EmitGauge("statefulsets.conditions", int64(1), map[string]string{
		"name":          "name1",
		"namespace":     "openshift",
	...
	err := mon.emitStatefulSetsConditions(ctx)
	if err != nil {
		t.Fatal(err)
	}
