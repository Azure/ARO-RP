...
func TestDetermineZones(t *testing.T) {
	for _, tt := range []struct {
	...
	}{
		{
			...
			wantErr:              "400: InvalidZoneConfiguration: properties.workerProfiles[0].VMSize: cluster creation with mix of zonal and non-zonal resources is unsupported (control plane zones: 0, worker zones: 3)",
		},
		{
			name:                  "non-zonal",
		...
		},
		{
			...
			workerSkuZones:       nil,
			wantErr:              "400: InvalidZoneConfiguration: properties.workerProfiles[0].VMSize: cluster creation with mix of zonal and non-zonal resources is unsupported (control plane zones: 3, worker zones: 0)",
		},
		{
		...
		},
		{
			...
			workerSkuZones:       []string{"1", "2", "3"},
			wantErr:              "400: InvalidZoneConfiguration: properties.masterProfile.VMSize: control plane SKU 'controlPlaneSKU' only available in 2 zones, need 3",
		},
		{
		...
		},
		{
			...
			wantErr:              "400: InvalidZoneConfiguration: properties.workerProfiles[0].VMSize: worker SKU 'workerSKU' only available in 2 zones, need 3",
		},
		{
