...
func TestBalance(t *testing.T) {
	type test struct {
		name     string
		monitors []string
		doc      func() *api.BucketServiceDocument
		validate func(*testing.T, *test, *api.BucketServiceDocument)
	}
	...
	for _, tt := range []*test{
		{
			...
			monitors: []string{"one"},
			doc: func() *api.BucketServiceDocument {
				return &api.BucketServiceDocument{}
			},
			validate: func(t *testing.T, tt *test, doc *api.BucketServiceDocument) {
				for i, bucket := range doc.Buckets {
					if bucket != "one" {
						t.Error(i, bucket)
					}
				}
