...
// ResourceGroupCustomRoleAssignmentWithName returns a Resource granting roleID on the
// current resource group to spID.  Arguments spID and name must be valid ARM
// expressions, e.g. "'foo'" or "concat('foo')".  Use this function in ARM
// templates which have already been deployed, to preserve the name and avoid a
// RoleAssignmentExists error. Additionally a dependency is added on the role, use this function
// when the Role Definition is declared in the same ARM template at non-subscription scope.
func ResourceGroupCustomRoleAssignment(roleName, spID string, name string, condition ...interface{}) *arm.Resource {
	dependsOn := []string{
		"[resourceId('Microsoft.Authorization/roleDefinitions', guid(resourceGroup().id, '" + roleName + "'))]",
	}
	return resourceGroupRoleAssignmentWithDetails("guid(resourceGroup().id, '"+roleName+"')", spID, name, dependsOn, false, condition...)
}

// ResourceRoleDefinition returns a Resource defining a role granting the required
// actions and dataActions, forbidding notActions and notDataActions. Argument name must be valid ARM expressions.
func ResourceRoleDefinition(name string, permissions []mgmtauthorization.Permission) *arm.Resource {
	r := &arm.Resource{
		Resource: mgmtauthorization.RoleDefinition{
			...
			RoleDefinitionProperties: &mgmtauthorization.RoleDefinitionProperties{
				...
				Description: to.StringPtr("This role has been automatically generated by the Azure Red Hat OpenShift Resource Provider"),
				RoleType:    to.StringPtr("CustomRole"),
				Permissions: &permissions,
				//the roles defined here can only be assigned inside the cluster resourceGroup
				AssignableScopes: &[]string{"[resourceGroup().id]"},
			},
		},
