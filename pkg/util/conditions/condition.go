...
func IsTrue(conditions []operatorv1.OperatorCondition, t arov1alpha1.CondititionType) bool {
	return isCondition(conditions, t, operatorv1.ConditionTrue)
}

func IsFalse(conditions []operatorv1.OperatorCondition, t arov1alpha1.CondititionType) bool {
	return isCondition(conditions, t, operatorv1.ConditionFalse)
}

func isCondition(conditions []operatorv1.OperatorCondition, t arov1alpha1.CondititionType, s operatorv1.ConditionStatus) bool {
	for _, condition := range conditions {
		if condition.Type == t.String() && condition.Status == s {
			return true
		}
	}
	return false
...
func cleanStaleConditions(cluster *arov1alpha1.Cluster, role string) (changed bool) {
	...
	current := map[arov1alpha1.CondititionType]bool{}
	for _, ct := range arov1alpha1.AllConditionTypes() {
		current[ct] = true
	}

	for _, cond := range cluster.Status.Conditions {
		if _, ok := current[arov1alpha1.CondititionType(cond.Type)]; ok {
