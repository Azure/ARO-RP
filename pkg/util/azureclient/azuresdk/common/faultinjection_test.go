...
// TestLROPollFaultPolicyDo verifies that lroPollFaultPolicy observes real ARM 202 responses,
// records the real poll URL, and injects a 200 Failed when that poll URL is subsequently GET-ted.
// The real response is never mutated.
func TestLROPollFaultPolicyDo(t *testing.T) {
	...
	t.Run("real ARM 202 triggers LRO poll injection on real poll URL", func(t *testing.T) {
		srv, close := testhttp.NewServer()
		defer close()
		...
		// The 202 must carry the real poll URL; the policy must not mutate the response.
		if got := putResp.Header.Get("Azure-AsyncOperation"); got != realPollURL {
			t.Errorf("PUT response: Azure-AsyncOperation = %q, want real URL %q", got, realPollURL)
		}
		putResp.Body.Close()

		pollReq, err := runtime.NewRequest(context.Background(), http.MethodGet, realPollURL)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("non-202 response does not register poll injection", func(t *testing.T) {
		srv, close := testhttp.NewServer()
		defer close()
		// ARM returns synchronous 200 (not an LRO); subsequent GET passes through normally.
		srv.AppendResponse(testhttp.WithStatusCode(http.StatusOK))
		srv.AppendResponse(testhttp.WithStatusCode(http.StatusOK))
	...
	t.Run("second PUT to same URL passes through unmodified (retry path)", func(t *testing.T) {
		srv, close := testhttp.NewServer()
		defer close()

		realPollURL := srv.URL() + "/subscriptions/sub/providers/Microsoft.Compute/operations/op1"

		// ARM returns 202 on both PUTs (same URL, two attempts).
		srv.AppendResponse(
			testhttp.WithStatusCode(http.StatusAccepted),
			testhttp.WithHeader("Azure-AsyncOperation", realPollURL),
		)
		...
		// ARM returns 200 Succeeded on the second poll (retry succeeds).
		srv.AppendResponse(testhttp.WithStatusCode(http.StatusOK))
		...
		writeURL := srv.URL() + "?resource=r"

		put := func() *http.Response {
			t.Helper()
			req, err := runtime.NewRequest(context.Background(), http.MethodPut, writeURL)
			if err != nil {
				t.Fatal(err)
			}
			resp, err := pl.Do(req)
		...
		get := func(url string) *http.Response {
			t.Helper()
			req, err := runtime.NewRequest(context.Background(), http.MethodGet, url)
			if err != nil {
				t.Fatal(err)
			}
			resp, err := pl.Do(req)
		...
		// Attempt 1: injection fires on the poll.
		resp1 := put()
		defer resp1.Body.Close()
		if resp1.StatusCode != http.StatusAccepted {
			t.Fatalf("attempt 1 PUT: got %d, want 202", resp1.StatusCode)
		}
		if got := resp1.Header.Get("Azure-AsyncOperation"); got != realPollURL {
			t.Errorf("attempt 1 PUT: Azure-AsyncOperation = %q, want real URL %q (response must not be mutated)", got, realPollURL)
		}

		poll1 := get(realPollURL)
		defer poll1.Body.Close()
		if poll1.StatusCode != http.StatusOK {
			t.Errorf("attempt 1 poll: got %d, want 200 (injected failure)", poll1.StatusCode)
		}

		// Attempt 2: write already marked, poll passes through to server.
		resp2 := put()
		defer resp2.Body.Close()
		if resp2.StatusCode != http.StatusAccepted {
			t.Fatalf("attempt 2 PUT: got %d, want 202", resp2.StatusCode)
		}

		poll2 := get(realPollURL)
		defer poll2.Body.Close()
		if poll2.StatusCode != http.StatusOK {
			t.Errorf("attempt 2 poll: got %d, want 200 (real ARM success)", poll2.StatusCode)
		}

		// Server received: PUT1, PUT2, GET(poll2); poll1 GET was intercepted.
		if got := srv.Requests(); got != 3 {
			t.Errorf("server received %d requests, want 3 (PUT1, PUT2, poll2 GET)", got)
		}
	})
}
