...
type operationsClient struct {
	mgmtredhatopenshift20210901preview.OperationsClient
}
...
func NewOperationsClient(environment *azureclient.AROEnvironment, subscriptionID string, authorizer autorest.Authorizer) OperationsClient {
	var client mgmtredhatopenshift20210901preview.OperationsClient
	if env.IsLocalDevelopmentMode() {
		client = mgmtredhatopenshift20210901preview.NewOperationsClientWithBaseURI("https://localhost:8443", subscriptionID)
		...
		client = mgmtredhatopenshift20210901preview.NewOperationsClientWithBaseURI(environment.ResourceManagerEndpoint, subscriptionID)
