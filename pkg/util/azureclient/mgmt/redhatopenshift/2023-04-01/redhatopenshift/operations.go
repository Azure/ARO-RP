...
type operationsClient struct {
	mgmtredhatopenshift20230401.OperationsClient
}
...
func NewOperationsClient(environment *azureclient.AROEnvironment, subscriptionID string, authorizer autorest.Authorizer) OperationsClient {
	var client mgmtredhatopenshift20230401.OperationsClient
	if env.IsLocalDevelopmentMode() {
		client = mgmtredhatopenshift20230401.NewOperationsClientWithBaseURI("https://localhost:8443", subscriptionID)
		...
		client = mgmtredhatopenshift20230401.NewOperationsClientWithBaseURI(environment.ResourceManagerEndpoint, subscriptionID)
