...
func (c *AROController) GetDesiredVersion(ctx context.Context) (*version.Version, error) {
	...
	clusterVersion, err := version.GetDesiredVersion(cv)
	if err != nil {
		c.Log.Errorf("error getting the desired OpenShift version: %v", err)
		return nil, err
	}
