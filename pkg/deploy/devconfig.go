...
func DevConfig(_env env.Core) (*Config, error) {
	...
	if azurePrefix == "" {
		azurePrefix = os.Getenv("USER")
	}

	keyvaultPrefix := azurePrefix + "-aro-" + _env.Location()
	if len(keyvaultPrefix) > 20 {
		keyvaultPrefix = keyvaultPrefix[:20]
	}

	oidcStorageAccountName := azurePrefix + _env.Location()
		...
		RPs: []RPConfig{
			{
				...
				GatewayResourceGroupName: azurePrefix + "-gwy-" + _env.Location(),
				RPResourceGroupName:      azurePrefix + "-aro-" + _env.Location(),
				Configuration: &Configuration{
					AzureCloudName:              &_env.Environment().ActualCloudName,
					DatabaseAccountName:         to.StringPtr(azurePrefix + "-aro-" + _env.Location()),
					GatewayStorageAccountDomain: to.StringPtr(azurePrefix + "gwy" + _env.Location() + ".blob." + _env.Environment().StorageEndpointSuffix),
					KeyvaultDNSSuffix:           &_env.Environment().KeyVaultDNSSuffix,
					KeyvaultPrefix:              &keyvaultPrefix,
					StorageAccountDomain:        to.StringPtr(azurePrefix + "aro" + _env.Location() + ".blob." + _env.Environment().StorageEndpointSuffix),
					OIDCStorageAccountName:      to.StringPtr(oidcStorageAccountName),
				},
			},
		},
		Configuration: &Configuration{
			ACRResourceID:                to.StringPtr("/subscriptions/" + _env.SubscriptionID() + "/resourceGroups/" + azurePrefix + "-global/providers/Microsoft.ContainerRegistry/registries/" + azurePrefix + "aro"),
			...
			ClusterParentDomainName:      to.StringPtr(azurePrefix + "-clusters." + os.Getenv("PARENT_DOMAIN_NAME")),
			...
			FluentbitImage:       to.StringPtr(version.FluentbitImage(azurePrefix + "aro." + _env.Environment().ContainerRegistryDNSSuffix)),
			...
			GlobalResourceGroupName:     to.StringPtr(azurePrefix + "-global"),
			...
			RPImagePrefix:                     to.StringPtr(azurePrefix + "aro.azurecr.io/aro"),
			...
			RPParentDomainName:                to.StringPtr(azurePrefix + "-rp." + os.Getenv("PARENT_DOMAIN_NAME")),
			RPVersionStorageAccountName:       to.StringPtr(azurePrefix + "rpversion"),
			...
			SubscriptionResourceGroupName:     to.StringPtr(azurePrefix + "-subscription"),
