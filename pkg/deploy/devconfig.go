...
func DevConfig(_env env.Core) (*Config, error) {
	...
	return &Config{
		...
		Configuration: &Configuration{
			...
			GatewayKustoClusterURI:      pointerutils.ToPtr(""),
			GatewayKustoDBName:          pointerutils.ToPtr(""),
			GatewayKustoIngestionType:   pointerutils.ToPtr("queued"),
			GatewayKustoLogsTableName:   pointerutils.ToPtr(""),
			GatewayKustoManagedIdentityClientID: pointerutils.ToPtr(""),
