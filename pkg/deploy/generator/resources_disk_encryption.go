...
func (g *generator) diskEncryptionKey() *arm.Resource {
	key := &mgmtkeyvault.Key{
		...
		Name:     to.StringPtr("[concat(parameters('kvName'), '/', resourceGroup().name, '-', 'disk-encryption-key')]"),
...
func (g *generator) diskEncryptionSet() *arm.Resource {
	diskEncryptionSet := &mgmtcompute.DiskEncryptionSet{
		EncryptionSetProperties: &mgmtcompute.EncryptionSetProperties{
			ActiveKey: &mgmtcompute.KeyVaultAndKeyReference{
				KeyURL: to.StringPtr("[reference(resourceId('Microsoft.KeyVault/vaults/keys', parameters('kvName'), concat(resourceGroup().name, '-', 'disk-encryption-key')), '2019-09-01', 'Full').properties.keyUriWithVersion]"),
	...
	return &arm.Resource{
		...
		Condition:  to.StringPtr("[parameters('ci')]"),
		DependsOn:  []string{"[resourceId('Microsoft.KeyVault/vaults/keys', parameters('kvName'), concat(resourceGroup().name, '-', 'disk-encryption-key'))]"},
	}
}
