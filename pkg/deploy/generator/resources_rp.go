...
func (g *generator) rpRBAC() []*arm.Resource {
	return []*arm.Resource{
		...
		rbac.ResourceRoleAssignmentWithName(
			...
			"concat(resourceGroup().location, parameters('environmentSuffix'), '.', parameters('clusterParentDomainName'), '/Microsoft.Authorization/', guid(resourceId('Microsoft.Network/dnsZones', concat(resourceGroup().location, '.', parameters('clusterParentDomainName'))), 'FP / DNS Zone Contributor'))",
		),
	}
}
