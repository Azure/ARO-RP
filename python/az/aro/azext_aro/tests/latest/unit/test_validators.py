...
test_validate_cidr_data = [
    ...
    ("should raise InvalidArgumentValueError when non valid IPv4 address due to being a simple string", Mock(key='this is an invalid network'), "key", InvalidArgumentValueError),
...
test_validate_subnet_data = [
    ...
    (
        ...
        'key',
        True,
        {"subscription": "subscription", "namespace": "MICROSOFT.NETWORK", "type": "virtualnetworks", "last_child_num": 1, "child_namespace_1": "something"},
        "subscription",
        None,
        Mock(cli_ctx=None),
        InvalidArgumentValueError
    ...
    (
        ...
        'key',
        True,
        {"subscription": "subscription", "namespace": "MICROSOFT.NETWORK", "type": "virtualnetworks", "last_child_num": 1, "child_type_1": "this_should_be_subnets"},
        "subscription",
        None,
        Mock(cli_ctx=None),
        InvalidArgumentValueError
    ...
    (
        ...
        'key',
        True,
        {"subscription": "subscription", "namespace": "MICROSOFT.NETWORK", "type": "virtualnetworks", "last_child_num": 1, "child_type_1": "subnets"},
        "subscription",
    ...
    (
        ...
        'key',
        True,
        {"subscription": "subscription", "namespace": "MICROSOFT.NETWORK", "type": "virtualnetworks", "last_child_num": 1, "child_type_1": "subnets", "resource_group": None, "name": None, "child_name_1": None},
        "subscription",
...
test_validate_subnets_data = [
    ("should raise InvalidArgumentValueError exception when resource group of master_parts is different than resource_group of worker_parts", [{"resource_group": "a"}, {"resource_group": "b"}], InvalidArgumentValueError),
    ("should raise InvalidArgumentValueError exception when name of master_parts is different than name of worker_parts", [{"resource_group": "equal", "name": "something"}, {"resource_group": "equal", "name": "different"}], InvalidArgumentValueError),
    ("should raise InvalidArgumentValueError exception when name of master_parts is different than name of worker_parts", [{"resource_group": "equal", "name": "something", "child_name_1": "should_not_be_equal"}, {"resource_group": "equal", "name": "something", "child_name_1": "should_not_be_equal"}], InvalidArgumentValueError),
    ("should not raise any exception", [{"resource_group": "equal", "name": "something", "child_name_1": "a"}, {"resource_group": "equal", "name": "something", "child_name_1": "z"}], None)
]


@pytest.mark.parametrize("test_description, parse_resource_id_mock_tc, expected_exception", test_validate_subnets_data, ids=[i[0] for i in test_validate_subnets_data])
@patch('azext_aro._validators.parse_resource_id')
...
test_validate_visibility_data = [
    ("should not raise any exception", "key", Mock(key=None), None),
    ("should raise InvalidArgumentValueError exception because visibility is not one of the expected values", "key", Mock(key="super_private"), InvalidArgumentValueError),
    ("should not raise any exception because visibility is private", "key", Mock(key="private"), None),
    ("should not raise any exception because visibility is PRIVATE", "key", Mock(key="PRIVATE"), None),
    ("should not raise any exception because visibility is PUBLIC", "key", Mock(key="PUBLIC"), None),
    ("should not raise any exception because visibility is public", "key", Mock(key="public"), None)
]


@pytest.mark.parametrize("test_description, key, namespace, expected_exception", test_validate_visibility_data, ids=[i[0] for i in test_validate_visibility_data])
...
test_validate_vnet_resource_group_name_data = [
    ("should not copy namespace.resource_group_name to namespace.vnet_resource_group_name because namespace.resource_group_name already has a value", Mock(vnet_resource_group_name="hello", resource_group_name="this_will_not_be_copied"), "hello"),
    ("should copy resource_group_name field to vnet_resource_group_name because namespace.vnet_resource_group_name is None", Mock(vnet_resource_group_name=None, resource_group_name="will_copy_this"), "will_copy_this")
]


@pytest.mark.parametrize("test_description, namespace, expected_namespace_vnet_resource_group_name", test_validate_vnet_resource_group_name_data, ids=[i[0] for i in test_validate_vnet_resource_group_name_data])
def test_validate_vnet_resource_group_name(test_description, namespace, expected_namespace_vnet_resource_group_name):
    ...
    assert(namespace.vnet_resource_group_name == expected_namespace_vnet_resource_group_name)
...
@pytest.mark.parametrize("test_description, namespace, expected_exception", test_validate_worker_count_data, ids=[i[0] for i in test_validate_worker_count_data])
...
test_validate_worker_vm_disk_size_gb_data = [
    ("should not raise any Exception because worker_vm_disk_size_gb of namespace is None", Mock(worker_vm_disk_size_gb=None), None),
    ("should raise InvalidArgumentValueError Exception because worker_vm_disk_size_gb of namespace is less than minimum_worker_vm_disk_size_gb", Mock(worker_vm_disk_size_gb=2), InvalidArgumentValueError),
    ("should not raise any Exception because worker_vm_disk_size_gb of namespace is equal than minimum_worker_vm_disk_size_gb", Mock(worker_vm_disk_size_gb=128), None),
    ("should not raise any Exception because worker_vm_disk_size_gb of namespace is greater than minimum_worker_vm_disk_size_gb", Mock(worker_vm_disk_size_gb=220), None)
]


@pytest.mark.parametrize("test_description, namespace, expected_exception", test_validate_worker_vm_disk_size_gb_data, ids=[i[0] for i in test_validate_worker_vm_disk_size_gb_data])
...
test_validate_refresh_cluster_credentials_data = [
    ("should not raise any Exception because namespace.refresh_cluster_credentials is none", Mock(refresh_cluster_credentials=None), None),
    ("should raise RequiredArgumentMissingError Exception because namespace.client_secret is not None", Mock(client_secret="secret_123"), RequiredArgumentMissingError),
    ("should raise RequiredArgumentMissingError Exception because namespace.client_id is not None", Mock(client_id="client_id_456"), RequiredArgumentMissingError),
    ("should not raise any Exception because namespace.client_secret is None and namespace.client_id is None", Mock(client_secret=None, client_id=None), None)
]


@pytest.mark.parametrize("test_description, namespace, expected_exception", test_validate_refresh_cluster_credentials_data, ids=[i[0] for i in test_validate_refresh_cluster_credentials_data])
